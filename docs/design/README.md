...
27. [**26-default-schemas-and-probes.md**](./26-default-schemas-and-probes.md) — Eight default per-format schemas (`text`, `markdown`, `png`, `jpeg`, `mp3`, `wav`, `mp4`, `pdf`) plus six new structural probes (`text.line_count`, `image.dimensions`, `audio.duration_seconds`, `video.duration_seconds`, `video.dimensions`, `pdf.page_count`) ship as code. MP4 reads duration + dimensions via an inline ISO-BMFF box parser. No `user_provided` fields in the defaults; `text` and `markdown` use `allow_extra_fields` for LLM-attached annotations.
...
| 26-default-schemas-and-probes | Eight default per-format schemas + six new structural probes (line count, image dimensions, audio duration, video duration + dimensions, PDF page count) ship as code; no user_provided fields by default. |
